...
export default function PositionsPage() {
  ...
  } = useSWR<{ positionId: number; name: string }[]>("/api/positions", fetcher);
  ...
  const handleAddPosition = async () => {
    ...
    try {
      await post(
        "/api/positions",
        JSON.stringify({ name: newPositionName.trim() })
      );
  ...
  const handleEditPosition = async (id: number) => {
    try {
      await put(
        `/api/positions/${id}`,
        JSON.stringify({ name: editPositionName })
      );
      mutate("/api/positions");
                      ...
                      <td className="px-6 text-sm text-gray-900">
                        {editingPositionId === position.positionId ? (
                          <TextInput
                            ...
                            onChange={(e) =>
                              setEditPositionName(e.currentTarget.value)
                            }
                        ...
                        {editingPositionId === position.positionId ? (
                          <>
                            <ActionButton
                              kind="check"
                              onClick={() =>
                                handleEditPosition(position.positionId)
                              }
                            />
                            <ActionButton
                              kind="cancel"
                    ...
                    <tr className="bg-blue-50">
                      <td className="px-6 py-3">
                        <TextInput
                          ...
                          onChange={(e) =>
                            setNewPositionName(e.currentTarget.value)
                          }
                          placeholder="Nhập tên chức vụ"
                          autoFocus
                        />
